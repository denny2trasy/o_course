class CoursePackagesController < ApplicationController
  ...
  def new
    @course_package = CoursePackage.new(:customer_id=>@customer.id)
  end
  ...
  def create
    ...
    #end
    redirect_to course_packages_path(:q => {:customer_id => params[:course_package][:customer_id]})
  end
  ...
  def destroy
    ...
    redirect_to course_packages_path(:q => {:customer_id => @customer.id})
  end
